...
工程師的主要任務是將需求轉化為可執行的程式碼，專注於實現功能。然而，開發過程中不可避免地會有邏輯錯誤或疏漏，這些錯誤並非因為工程師不負責任或缺乏技能，而更多是由於視角的侷限或對需求的誤解。

**沒有工程師會故意製造 BUG**，因為每一個 BUG 都意味著更多的返工與額外的壓力。實際上，工程師希望程式一次性順利運行，並為團隊節省時間。然而，一些問題的產生是難以完全避免的，原因可能包括以下幾點：

1. **缺乏對需求的全局理解**  
   在軟體專案中，需求往往來自不同的利益相關者，而需求文件可能在傳遞過程中遺漏細節或缺乏清晰定義。工程師基於片段資訊進行開發，可能導致功能與實際需求不符。例如，開發一個報表匯出功能時，工程師可能忽略了用戶需要特定的檔案格式。

2. **假設用戶會以「預期方式」使用軟體**  
   工程師在開發時，通常會依據既定的需求編寫測試案例。然而，用戶的實際使用方式往往超出預期。例如：
   - 工程師假設用戶只會輸入合法數字，但用戶可能會輸入特殊字符，導致程式崩潰。
   - 工程師未考慮到同時多用戶操作的情況，導致併發處理出現問題。

3. **對特定技術的依賴或偏好**  
   工程師可能基於自己熟悉的技術棧進行開發，但有時候選擇的工具或框架可能並不完全適合需求。例如，為了快速交付，可能選擇了簡單的現成解決方案，但忽略了其對特殊場景的局限性。

4. **邊界情境的不可預測性**  
   某些 BUG 來自於系統的極端邊界情境。例如，在處理大量數據時，程式可能會因資源不足而導致性能瓶頸，這些問題通常在測試階段不易被察覺。

沒有工程師希望看到自己的程式出現問題，但由於軟體開發本身的複雜性與不確定性，盲點的存在是不可避免的。透過團隊內部的協作與有效溝通，可以補足這些盲點，並在未來的專案中逐步提升品質。

![工程師難](/career/project/工程師難.png "工程師難")

### PM 的盲點

PM 的職責是規劃與管理專案，但需求變更幾乎是軟體專案的常態。PM 的盲點在於過度依賴初期規格書，未能靈活適應變更。例如：
- 高估需求的穩定性，忽略市場或用戶需求的動態變化。
- 過於追求計劃執行，而未考慮靈活調整的必要性。

### 設計師的盲點

設計師可能會過於專注於美感與細節，忽略了用戶的實際需求與使用情境。例如：
- 將產品界面設計得過於複雜，導致用戶無法快速找到關鍵功能。
- 過度追求創新，忽略了用戶的學習成本和直覺操作。

### 高階主管的盲點

高階主管往往希望透過會議了解專案進度，但過多的會議可能產生「虛假的進展感」。這種盲點體現在：
- 將會議視為行動，而非促進行動的工具。
- 忽略團隊需要更多專注時間來解決實際問題。
