...
function drawMortgageChart() {
    ...
    const { totalPriceEstimated, } = props.estatePrice
    if (!totalPriceEstimated || !loanPercent || !loanTerm) {
        return
    }
    const loanAmount = totalPriceEstimated * loanPercent / 100
    mortgage.value.loanAmount = Math.floor(loanAmount)
    const downPayment = Math.floor(totalPriceEstimated - loanAmount)
