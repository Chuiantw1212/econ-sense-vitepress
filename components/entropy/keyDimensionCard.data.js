const MANUAL_DB = {
    ...
    x: {
        ...
        neg: { // O: Others (Oxytocin)
            ...
            mechanism: `你的大腦受 催產素 調節，促進了極高的 「跨半球連結 (Inter-hemispheric Connectivity)」。連結左腦（語言/邏輯）與右腦（直覺/空間）的高效整合，使你的大腦呈現 「全腦整合」 狀態。你的優勢在於能同時處理多維度訊號，進行高層次的社會心智判讀（讀懂空氣與社交訊號），這是建立深層信任與維護群體穩定的生物學基礎。`,
...
const AXIS_CONFIG = {
    ...
    y: {
        name: '拓撲向',
        pos: { label: '實感執行 (R)', color: '#909399', icon: '👁️', manual: MANUAL_DB.y.pos },
        neg: { label: '預判模擬 (V)', color: '#9370DB', icon: '🧠', manual: MANUAL_DB.y.neg }
    },
    z: {
        name: '熵狀態',
        pos: { label: '熱系統 (H)', color: '#E6A23C', icon: '🔥', manual: MANUAL_DB.z.pos },
        neg: { label: '冷系統 (C)', color: '#409EFF', icon: '❄️', manual: MANUAL_DB.z.neg }
    },
};
